...
- (void)insertText:(NSString *)text {
    ...
    if ([self.delegate respondsToSelector:@selector(unitField:shouldChangeCharactersInRange:replacementString:)]) {
        if ([self.delegate unitField:self shouldChangeCharactersInRange:NSMakeRange(_string.count, 0) replacementString:text] == NO) {
            return;
        }
    }
...
- (void)deleteBackward {
    ...
    if ([self.delegate respondsToSelector:@selector(unitField:shouldChangeCharactersInRange:replacementString:)]) {
        if ([self.delegate unitField:self shouldChangeCharactersInRange:NSMakeRange(_string.count -1, 1) replacementString:@""] == NO) {
            return;
        }
    }
