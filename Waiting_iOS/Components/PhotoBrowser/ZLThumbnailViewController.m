...
//    [self setupBottomView];   // 隐藏底部工具栏  change 1
...
- (void)viewDidLayoutSubviews
{
    ...
    showBottomView = NO;    // 改为不展示底部工具条  change 1
    ZLImageNavigationController *nav = (ZLImageNavigationController *)self.navigationController;
...
//    nav.isSelectOriginalPhoto = self.btnOriginalPhoto.selected;
    nav.isSelectOriginalPhoto = YES;    // 直接设置为已选择原图 change 5 finish!
    if (nav.isSelectOriginalPhoto) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self btnOriginalPhoto_Click:nil];   // 直接获取原图 change 2
    
    [self btnDone_Click:nil];   // 直接返回 change 3
    /** // 取消掉进入详情页，直接返回  change 4
