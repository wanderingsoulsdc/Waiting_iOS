...
//    NSString* str = [dic modelToJSONString];
    ...
    if (self.callInfo.callType == NIMNetCallTypeVideo) {
        //对比度强度 [0-4] 默认为 1
        option.videoCaptureParam.videoProcessorParam.contrastIntensity = 1;
        //磨皮强度 [0-1] 默认为 0
        option.videoCaptureParam.videoProcessorParam.smoothIntensity = 0;
    }
...
- (void)onResponse:(UInt64)callID from:(NSString *)callee accepted:(BOOL)accepted{
    if (self.callInfo.callID == callID) {
        if (!accepted) {
            self.chatRoom = nil;
            [SVProgressHUD showErrorSVP:@"对方拒绝接听"];
...
- (void)fillUserSetting:(NIMNetCallOption *)option
{
    ...
    option.serverRecord.enableServerAudioRecording = [[NTESBundleSetting sharedConfig] serverRecordAudio];
    option.serverRecord.enableServerVideoRecording = [[NTESBundleSetting sharedConfig] serverRecordVideo];
