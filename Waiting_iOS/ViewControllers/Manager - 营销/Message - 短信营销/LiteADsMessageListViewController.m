//
//  LiteADsMessageListViewController.m
//  Waiting_iOS
//
...
@interface LiteADsMessageListViewController ()<UITableViewDelegate, UITableViewDataSource,PGDatePickerDelegate,XZPickViewDelegate, XZPickViewDataSource>

@property (nonatomic , strong) UIView               * topView;         //上部视图
@property (nonatomic , strong) UIButton             * dateButton;      //上部时间选择按钮
@property (nonatomic , strong) UILabel              * expenseLabel;    //上部短信花费
@property (nonatomic , strong) UILabel              * personNumLabel;  //上部通讯人数
@property (nonatomic , strong) UILabel              * expenseTitleLabel;//上部短信花费标题
...
@property (nonatomic , strong) UIView               * selectView;      //短信筛选视图
@property (nonatomic , strong) UIButton             * selectButton;    //短信筛选按钮
@property (nonatomic , strong) XZPickView           * pickView;        //筛选状态选择器
@property (nonatomic , strong) NSMutableArray       * selectArray;      //状态数组
@property (nonatomic , strong) NSString             * currentStatusStr; //当前状态

@property (nonatomic , strong) UITableView          * tableView;       //短信列表
@property (nonatomic , strong) NSMutableArray       * listDataArray;   //短信列表数据
@property (nonatomic , assign) NSInteger            currentPage;       //短信数据当前页数
@property (nonatomic , strong) NSString             * currentDateStr;  //短信请求所需时间字符串

@property (nonatomic , strong) BHLoadFailedView     * noDataView;      //短信无数据页面

@end

@implementation LiteADsMessageListViewController
...
- (void)messageAddOrEditSuccess:(NSNotification *)noti{
    NSString *messageName = noti.object;
    
    self.currentStatusStr = @"";//全部

    [self.selectButton setTitle:self.selectArray[0] forState:UIControlStateNormal];
    [self layoutCallStatusSelectButton];
    [self getCurrentDate];
    [self.pickView selectRow:0 inComponent:0 animated:YES];
    
    [[ShowHUDTool shareManager] showAlertWithCustomImage:@"login_register_success" title:@"短信创建成功" message:messageName];
}
    ...
    [FSNetWorkManager requestWithType:HttpRequestTypeGet
                     ...
                     withSuccessBlock:^(NSDictionary *object) {
                         ...
                         if (NetResponseCheckStaus)
                         {
                             ...
                             NSDictionary *reportDic = [dic objectForKey:@"report"];
                             self.personNumLabel.text = [reportDic stringValueForKey:@"num" default:@""];
                             self.expenseLabel.text = [reportDic stringValueForKey:@"reportSpend" default:@""];
                             ...
                             if (array.count > 0)
                             {
                                 for (NSDictionary * dict in array)
                                 {
                                     ...
                                     model.successNum = [dict stringValueForKey:@"successNum" default:@""];
                                     model.status = [dict stringValueForKey:@"status" default:@""];
                                     model.budget = [dict stringValueForKey:@"budget" default:@""];
...
{
    LiteADsMessageListCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass(self.class) forIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LiteADsMessageDetailViewController *detailVC = [[LiteADsMessageDetailViewController alloc] init];
...
- (UITableView *)tableView
{
    if (!_tableView)
    {
        ...
        [_tableView registerClass:[LiteADsMessageListCell class] forCellReuseIdentifier:NSStringFromClass(self.class)];
    }
    return _tableView;
}
...
- (UILabel *)expenseTitleLabel{
    if (!_expenseTitleLabel){
        ...
        _expenseTitleLabel.text = @"短信花费（元）";
...
{
    if (!_selectArray)
    {   /*全部，发送中，发送成功，发送失败，审核中，审核拒绝*/
        _selectArray = [NSMutableArray arrayWithObjects:@"全部",@"发送中",@"发送成功",@"发送失败",@"审核中",@"审核拒绝",nil];
    }
    return _selectArray;
}
